...
async function guardarPresupuestoCategoria() {
    ...
    const datosPresupuesto = { mes, categoria, monto: Number(monto.toFixed(2)) };
...
async function eliminarPresupuesto(categoria) {
    ...
    try {
        let urlDelete = presupuestoInfo.id 
            ? `${SUPABASE_URL}/rest/v1/presupuestos?id=eq.${encodeURIComponent(presupuestoInfo.id)}`
