...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Inicializamos las fechas del filtro con el mes actual por defecto
    if (filtroDesde && filtroHasta) {
        filtroDesde.value = `${mesActualStr}-01`;
        // Calculamos el último día del mes actual
        const [anio, mes] = mesActualStr.split('-');
        const ultimoDia = new Date(anio, mes, 0).getDate();
        filtroHasta.value = `${mesActualStr}-${ultimoDia}`;
    ...
    if (presupuestoMesSelector) {
        ...
        presupuestoMesSelector.addEventListener('change', () => {
            const mesSeleccionado = presupuestoMesSelector.value; // Formato YYYY-MM
            if (filtroDesde && filtroHasta && mesSeleccionado) {
                filtroDesde.value = `${mesSeleccionado}-01`;
                
                const [anio, mes] = mesSeleccionado.split('-');
                const ultimoDia = new Date(anio, mes, 0).getDate();
                filtroHasta.value = `${mesSeleccionado}-${ultimoDia}`;
            }
            // Forzamos la sincronización completa tras cambiar el selector de mes
