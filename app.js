...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    configurarRealtimeSupabase(); // <--- Activamos la sincronización en tiempo real
    ...
    if (filtroMes) {
        filtroMes.value = mesActualStr;
        filtroMes.addEventListener('change', () => sincronizarDatosGlobales());
    }
...
// Sincronización automática en tiempo real entre el teléfono y la PC
function configurarRealtimeSupabase() {
    ...
    // Conexión genérica para escuchar cambios en las tablas de Supabase sin romper la app si falla el WS
    try {
        ...
        ws.onmessage = async (event) => {
            try {
                ...
                if (data.event === "INSERT" || data.event === "UPDATE" || data.event === "DELETE") {
                    // Si ocurre cualquier cambio en la nube, refrescamos los datos automáticamente
                    await cargarDatosNubeSilencioso();
                }
            } catch (err) {}
        };
    } catch (e) {
        console.warn("Realtime nativo no disponible, operando con consultas normales.");
    }
}
    ...
    try {
        ...
        if (resPres.ok) {
            ...
            dataPresupuestos.forEach(p => {
                ...
                presupuestosMensuales[p.mes][p.categoria] = { monto: Number(p.monto), id: p.id };
            });
        }
        sincronizarDatosGlobales();
    } catch (e) {}
...
function sincronizarDatosGlobales() {
    const filtroMesElem = document.getElementById('filtro-mes');
    const presupuestoMesElem = document.getElementById('presupuesto-mes-selector');

    if (filtroMesElem && presupuestoMesElem && filtroMesElem.value !== presupuestoMesElem.value) {
        presupuestoMesElem.value = filtroMesElem.value;
    }
...
async function eliminarMovimiento(idMovimiento) {
    try {
        ...
        if (response.ok) {
            ...
            sincronizarDatosGlobales();
            mostrarNotificacion('Movimiento eliminado de la base de datos', 'info');
        } else {
            let errText = await response.text();
            console.error("Error al eliminar movimiento:", errText);
            mostrarNotificacion('No se pudo eliminar el movimiento', 'error');
        }
    } catch (e) {
        console.error("Error de red al eliminar movimiento:", e);
    }
}
...
async function guardarPresupuestoCategoria() {
    ...
    try {
        let headersUpsert = { ...SUPABASE_HEADERS, "Prefer": "resolution=merge-duplicates,return=representation" };
        ...
        if (response.ok) {
            ...
            let idReg = (Array.isArray(dataReg) && dataReg.length > 0) ? dataReg[0].id : null;

            if (!presupuestosMensuales[mes]) presupuestosMensuales[mes] = {};
            presupuestosMensuales[mes][categoria] = {
                monto: monto,
                id: idReg
            };
            ...
            let errText = await response.text();
            console.error("Error presupuesto Supabase:", errText);
            mostrarNotificacion('Error al guardar presupuesto', 'error');
        }
    } catch (e) {
        console.error("Error de red presupuesto:", e);
    }
}
...
async function eliminarPresupuesto(categoria) {
    ...
    if (!presupuestoInfo) return;
    const idPresupuesto = presupuestoInfo.id;

    try {
        let urlDelete = idPresupuesto 
            ? `${SUPABASE_URL}/rest/v1/presupuestos?id=eq.${encodeURIComponent(idPresupuesto)}`
            : `${SUPABASE_URL}/rest/v1/presupuestos?mes=eq.${mes}&categoria=eq.${encodeURIComponent(categoria)}`;
        ...
        if (response.ok) {
            if (presupuestosMensuales[mes]) {
                delete presupuestosMensuales[mes][categoria];
            }
            sincronizarDatosGlobales();
            mostrarNotificacion('Presupuesto eliminado de la base de datos', 'info');
        } else {
            let errText = await response.text();
            console.error("Error al eliminar presupuesto:", errText);
            mostrarNotificacion('No se pudo eliminar el presupuesto', 'error');
        }
    } catch (e) {
        console.error("Error de red al eliminar presupuesto:", e);
    }
}
...
function agregarNuevaCategoria() {
    ...
    if (categoriasBase[tipo].includes(nombre)) {
        mostrarNotificacion('La categoría ya existe', 'info');
        return;
    }
...
function actualizarTablaYResumen() {
    ...
    const mesSeleccionado = document.getElementById('filtro-mes') ? document.getElementById('filtro-mes').value : '';
    let movimientosVisibles = movimientos;

    if (mesSeleccionado) {
        movimientosVisibles = movimientos.filter(m => m.fecha && m.fecha.startsWith(mesSeleccionado));
    }
...
function calcularYMostrarResumenGeneral() {
    ...
    if (!lblIngresos && !lblGastos && !lblDisponible) return; 

    const mesSeleccionado = document.getElementById('filtro-mes') ? document.getElementById('filtro-mes').value : mesActualStr;
    let movimientosVisibles = movimientos;
...
function renderizarProgresoCategoriasGeneral() {
    ...
    let movimientosVisibles = movimientos.filter(m => m.tipo === 'Gasto' && m.fecha && m.fecha.startsWith(mesSel));
