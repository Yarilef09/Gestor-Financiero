...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const filtroFechaUnica = document.getElementById('filtro-fecha-unica'); // Nuevo input único de día exacto
    ...
    // Inicializamos el selector de fecha del mes/presupuesto por defecto
    ...
    // Filtro por un día específico (limpio y sin rangos confusos)
    if (filtroFechaUnica) {
        filtroFechaUnica.value = ''; // Por defecto vacío para mostrar todo o manejarse con el mes
        filtroFechaUnica.addEventListener('change', () => {
            sincronizarDatosGlobales();
        });
    }
...
function setupEventListeners() {
    ...
    if (btnQuitarFiltro) {
        btnQuitarFiltro.addEventListener('click', () => {
            const filtroFechaUnica = document.getElementById('filtro-fecha-unica');
            if (filtroFechaUnica) filtroFechaUnica.value = '';
            sincronizarDatosGlobales();
        });
    }
...
async function eliminarMovimiento(idMovimiento) {
    try {
        let response = await fetch(`${SUPABASE_URL}/rest/v1/movimientos?or=(id.eq.${encodeURIComponent(idMovimiento)},ID.eq.${encodeURIComponent(idMovimiento)})`, {
            method: 'DELETE',
            headers: SUPABASE_HEADERS
        });
...
function obtenerMovimientosFiltrados() {
    const fechaUnica = document.getElementById('filtro-fecha-unica') ? document.getElementById('filtro-fecha-unica').value : '';
    ...
    return movimientos.filter(m => {
        ...
        // Si hay una fecha exacta seleccionada en el filtro único, filtramos estrictamente por ese día
        if (fechaUnica) {
            return m.fecha === fechaUnica;
        }

        // Si no hay fecha exacta, filtramos por el mes seleccionado en el selector general
