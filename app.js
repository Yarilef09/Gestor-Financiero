    ...
    try {
        ...
        if (resPres.ok) {
            ...
            dataPresupuestos.forEach(p => {
                ...
                // Guardamos también el ID del presupuesto por si se requiere eliminar de la BD
                presupuestosMensuales[p.mes][p.categoria] = {
                    monto: Number(p.monto),
                    id: p.id // id en minúsculas según tu indicación
                };
            });
        }
    } catch (error) {
...
async function eliminarMovimiento(idMovimiento) {
    try {
        // Se usa la columna ID en mayúsculas como indicaste para movimientos
...
function actualizarTablaYResumen() {
    ...
    movimientosVisibles.forEach(m => {
        // Usamos m.ID en mayúsculas como identificador único para el botón de eliminar
        const idUnico = m.ID; 
