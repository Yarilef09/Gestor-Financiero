...
// Total en Bs de la esquina: Suma ingresos y resta gastos netos
...
function actualizarTablaYResumen() {
    ...
    movimientosVisibles.forEach((m, index) => {
        const idUnico = m.ID !== undefined ? m.ID : m.id;
        const tr = document.createElement('tr');
        
        const montoVesVal = m.montoVes !== undefined ? m.montoVes : (m.montoUsd * (m.tasa || 1));
        const descripcionLarga = m.descripcion || '';
        const esLarga = descripcionLarga.length > 35;
        ...
        tr.innerHTML = `
            ...
            <td data-label="Descripción">
                <div class="desc-container">
                    <div id="${descId}" class="desc-text ${esLarga ? 'truncated' : ''}">${descripcionLarga}</div>
                    ${esLarga ? `<button class="btn-ver-mas" onclick="toggleDescripcion('${descId}', this)">Ver más</button>` : ''}
                </div>
            </td>
...
// Función para alternar el texto largo de las notas/descripciones
function toggleDescripcion(idElemento, btn) {
